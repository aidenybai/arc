...
export function decodeFrame(buffer: string): { frame: Frame; size: number } | null {
...
export class WsConnection {
  ...
  constructor(conn: any, options: { mask?: boolean; initial?: string } = {}) {
    this.conn = conn;
  ...
  async readMessage(): Promise<WsMessage | null> {
    let assembling = "";
    let assemblingOp = 0;
    while (true) {
  ...
  async #send(opcode: number, payload: string): Promise<boolean> {
...
function base64(bytes: string): string {
  let out = "";
